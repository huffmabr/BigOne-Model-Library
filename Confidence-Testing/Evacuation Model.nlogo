...
to find-flockmates
  set flockmates other pedestrians in-radius (0.0059053 * 100) with [confidence > conf-follow]
end
...
to create-file
  file-open (word  ExportName ".csv")
  ;file-open (user-new-file)
  
  file-type ("Redestrian, ")
  file-type ("Evac?, ")
  file-type ("base-adult")
  file-type ("tourist?, ")
  file-type ("Child?, ")
  file-type ("Elderly?, ")
  file-type ("Driving?,  ")
...
to create-meta
  file-open (word  ExportName "_meta"  ".csv")
  file-type ("RSNUM, ")
  file-type ("Evac%, ")
  file-type ("Confidence%,")
  file-type ("Rgular Adults,")
  file-type ("tourist#, ")
  file-type ("Children#, ")
  file-type ("Elderly#, ")
  file-type ("Driving#,  ")
  file-type ("Mortality,")
  file-type ("Tsunami Case,")
  file-type ("Immediate Evac,")
  file-type ("Iteration")
  file-type "\n"
  file-close
...
to export-meta
  let combineset (turtle-set pedestrians cars)
  file-open (word  ExportName "_meta"  ".csv")
  file-write (count combineset)
  file-type (",  ")
  file-write ((count combineset with [evacuated? = true])/ (count combineset))
  file-type (",  ")
  ;let comineset (turtle-set (pedestrians cars))
  file-write (mean [confidence] of combineset)
  file-type (",  ")
  file-write (count combineset with [tourist? = 0 and child? = 0 and elder? = 0])
  file-type (",  ")
  file-write (count combineset with [tourist? = 1])
  file-type (",  ")
  file-write (count combineset with [child? = 1])
  file-type (",  ")
  file-write (count combineset with [elder? = 1])
  file-type (",  ")
  file-write (count cars)
  file-type (",  ")
  file-write ((count combineset with [color = red])/(count combineset))
  file-type (",  ")
  file-write (tsunami-case)
  file-type (",  ")
...
to export-data
  ...
  ;set-current-directory "C:\\1.2 BETA\\Data"
 file-open (word ExportName ".csv")
  
;  ask residents[
;   file-write (who)
;   file-type (",  ")
;   file-write (evacuated?)
;   file-type (",  ")
;   file-write(tourist?)
;   file-type (",    ")
;   file-write(child?)
...
;   ]
  ...
  ;file-type "\n"
   ...
   file-write (evacuated?)
   file-type (",  ")
   ifelse(elder? = 0 and tourist? = 0 and child? = 0)[
     file-write(1)
   ][ file-write(0)]
   file-type (",  ")
   file-write(tourist?)
...
;  set origins []
;  ask intersections [
;    ;CHANGED THIS, ORIGINS IS NOW COLLECTION OF ALL INTERSECTIONS
;    set origins lput self origins
;  ]
;
...
;;;;;;;;;;;;;;; Functions to load the Vertical evacuation shelters based on user input (Mouse Click) ;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;;;;;;;;;;;;; DEMOGRAPHICS ;;;;;;;;;;;;;;
  ...
  ask residents [
    ...
    ifelse tourist? = 1[
      set confidence random tourist-base
    ][
    set confidence random resident-base
    ]
    
    if child? = 1 [
      set confidence (confidence + random child-mod)
      set speed (0.75 * speed)
      set miltime (miltime * 1.25)
    ]
    
    if elder? = 1[
      set confidence (confidence + elder-mod)
      set speed (0.75 * speed)
      set miltime (miltime * 1.15)
    ]
    
    if elder? = 0 and child? = 0[
      set confidence (confidence + resident-mod)
    ]
   ...
   ;;;;;;;;;;;;;; GOALS ;;;;;;;;;;;;;;
    ...
    if confidence < conf-follow-thresh
    [   
      set LTgoal "flock"                ;;ped follows others
    ]
    
    if confidence < conf-lead-thresh and confidence >= conf-follow-thresh
    [   
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; ASK RESIDENTS ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
      ...
      [
        ...
        [
          ...
          [
            ...
            ifelse ( optimal != -1 and noisy-weight + confidence / 5 > random 100 + 1 )
  ...
  ask pedestrians with [moving? = false and LTgoal != "flock" and path != []]
  [
  ...
  ask pedestrians with [moving? = false and LTgoal = "flock"][
    find-flockmates
...
;      ]
      ...
      [  
        ...
        [
          ...
          ifelse distance closeEnough < 600 * 0.0059043               ;; arbitrary distance of 600 feet before using optimal pathing to a nearby shelter
 ...
 ;; halt dead or safe pedestrians
  ask pedestrians with [color = green or color = red]
  [stop]
