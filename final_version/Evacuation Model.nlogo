...
to find-flockmates
  set flockmates other pedestrians in-radius (0.0059053 * 100) with [confidence > 30]
end
...
;;;;;;;;;;;;;; DEMOGRAPHICS ;;;;;;;;;;;;;;
  ...
  ask residents [
    ...
    ifelse tourist? = 1[
      set confidence random 25
    ][
    set confidence random 50
    ]
    
    if child? = 1 [
      set confidence (confidence + random 25)
      set speed (0.75 * speed)
      set miltime (miltime * 1.25)
    ]
    
    if elder? = 1[
      set confidence (confidence + random 55)
      set speed (0.75 * speed)
      set miltime (miltime * 1.15)
    ]
    
    if elder? = 0 and child? = 0[
      set confidence (confidence + random 50)
    ]
...
117
Mortality (%)
count turtles with [color = red] / (count residents + count pedestrians) * 100
2
1
11
...
"" ""
PENS
"cars" 1.0 0 -16777216 true "" "plot count cars"
...
"" ""
PENS
"residents" 1.0 0 -16777216 true "" "plot count residents"
